...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.log(
        `✅ Found ${courseProducts.length} course products in database`,
      );
    } catch (dbError) {
    ...
    > = {
      "brodboken-2026": {
        ...
        price: 74.53, // 79 kr inkl 6% moms => exkl moms
        type: "book",
        vatRate: 0.06,
      },
      paskbuffe: {
    ...
    async function resolveCourseIdFromCartItem(
      ...
      else if (id.includes("energy") || id.includes("insulin"))
        keyword = "Energy";
      else if (id.includes("basic")) keyword = "Basics";
      else if (id.includes("flow") || id.includes("gut")) keyword = "Flow";
      else if (id.includes("hormone") || id.includes("hormon"))
        keyword = "Hormonell";

      // Fallback: match by name
      if (!keyword) {
        if (name.includes("prova på") || name.includes("prova-pa"))
          keyword = "Prova";
        else if (name.includes("energy") || name.includes("insulin"))
          keyword = "Energy";
        else if (name.includes("basic")) keyword = "Basics";
        else if (name.includes("flow") || name.includes("gut"))
          keyword = "Flow";
    ...
    for (const item of items) {
      try {
        ...
        if (!product) {
          ...
          console.error(
            "Available products:",
            Array.from(productMap.keys()).slice(0, 20),
          ); // Limit output
          return NextResponse.json(
    ...
    if (process.env.PAYMENTS_SIMULATE === "true") {
      ...
      const totalAmountKr = SveaCheckoutService.formatPriceFromMinorUnits(
      ...
      try {
        ...
        if (order && customer?.email) {
          const purchasedCourses = order.items.filter(
            (it) => it.type === "course",
          );
          console.log(
          ...
          await emailService.sendOrderConfirmation({
            ...
            courses: purchasedCourses.map((it) => ({
              name: it.name,
              price: it.price,
            })),
            ...(needsLoginCredentials && {
    ...
    try {
      ...
      console.log(
        `✅ Calculated totals: ${sveaItems.length} items, subtotal INKL. moms: ${subtotal} öre (${subtotal / 100} kr)`,
      );
    } catch (calcError) {
      console.error("❌ Error calculating order totals:", calcError);
      throw new Error(
        `Fel vid beräkning av orderbelopp: ${calcError instanceof Error ? calcError.message : "Okänt fel"}`,
      );
    }
    ...
    if (totalAfterDiscountInOre <= 0) {
      console.log(
        "💚 Free order detected after discount - bypassing Svea checkout",
        {
          subtotal,
          discountAmount,
        ...
        prisma.order.create({
          data: {
            ...
            metadata: {
              ...
              discountAmount:
                discountAmount > 0
                  ? SveaCheckoutService.formatPriceFromMinorUnits(
                      discountAmount,
                    )
                  : null,
              freeOrder: true,
      ...
      try {
        ...
        if (
          bookItems.length > 0 &&
          customerEmail &&
          !customerEmail.startsWith("guest-")
        ) {
          ...
          for (const book of bookItems) {
            ...
            if (existing) {
              console.log(
                "ℹ️ Ebook token already exists (skipping create+email):",
                { orderId, ebookId },
              );
              continue;
            }
          ...
          console.log(
            `✅ E-book delivery email sent (free order): ${customerEmail}`,
          );
        }
        console.log(
          `✅ E-book delivery flow done (free order): ${customerEmail}`,
        );
      } catch (e) {
        console.warn(
          "⚠️ Failed to send ebook email (free order, non-critical):",
          e,
        );
      }
      ...
      try {
        ...
        await emailService.sendOrderConfirmation({
          ...
          orderNumber: orderId,
          totalAmount: 0,
          courses: validatedItems
            .filter((i) => i.type === "course")
            .map((i) => ({ name: i.name, price: 0 })),
          loginCredentials:
            needsLoginCredentials && temporaryPassword
              ? {
                  email: customerEmail,
                  password: temporaryPassword,
      ...
      try {
        ...
        console.warn(
          "⚠️ Failed to add to Mailchimp (non-critical):",
          mailchimpError,
        );
      }
    ...
    if (
      !checkoutRequest.cart.items ||
      checkoutRequest.cart.items.length === 0
    ) {
    ...
    console.log(
      "checkoutRequest.cart.items length:",
      checkoutRequest.cart.items.length,
    );
    console.log(
      "checkoutRequest.cart.items:",
      JSON.stringify(checkoutRequest.cart.items, null, 2),
    );

    // CRITICAL DEBUG LOGGING
    console.log("🚨 SVEA CHECKOUT REQUEST DEBUG:");
    console.log(
      "Items being sent to Svea:",
      JSON.stringify(sveaItems, null, 2),
    );
    const totalInOre = sveaItems.reduce(
    ...
    console.log("📤 Creating Svea checkout order:", {
      ...
      totalAmount: SveaCheckoutService.formatPriceFromMinorUnits(
        subtotal - discountAmount,
      ),
    ...
    console.log(
      "📤 Sending request to Svea with items:",
      JSON.stringify(sveaItems, null, 2),
    );
    ...
    console.log(
      "📤 FULL Svea checkout request:",
      JSON.stringify(checkoutRequest, null, 2),
    );
    console.log(
      "📤 VERIFICATION - cart.items.length:",
      checkoutRequest.cart.items.length,
    );

    // Calculate expected total
    const expectedTotal = checkoutRequest.cart.items.reduce(
      (sum, item) => sum + item.unitPrice * item.quantity,
      0,
    );
    console.log(
      `💰 Expected total to charge: ${expectedTotal} öre (${expectedTotal / 100} kr, rundat upp till närmsta krona)`,
    );
    ...
    try {
      ...
      console.error("❌ Svea createOrder failed:", {
        ...
        checkoutRequest: {
          ...
          totalAmount: checkoutRequest.cart.items.reduce(
            (sum, item) => sum + item.unitPrice * item.quantity,
            0,
          ),
        },
    ...
    const discountRatio =
      discountAmount > 0 ? totalAmountInOre / subtotalBeforeDiscount : 1;
    ...
    console.log(
      "If this shows 22.95, but Svea shows 0.25, then we are sending KRONOR to Svea instead of ÖRE!",
    );
    ...
    try {
      ...
      itemsWithDiscountedPrice = validatedItems.map((item) => {
        ...
        const itemTotalInOreInclVAT =
          SveaCheckoutService.formatPriceToMinorUnits(
            item.price * (1 + itemVatRate),
          ) * item.quantity;
        // Apply discount ratio to get discounted total
        const discountedItemTotalInOre = Math.round(
          itemTotalInOreInclVAT * discountRatio,
        );
        // Convert back to SEK per unit (exkl. VAT for storage)
        const discountedPricePerUnit =
          SveaCheckoutService.formatPriceFromMinorUnits(
            discountedItemTotalInOre / item.quantity,
          ) /
          (1 + itemVatRate);
    ...
    if (appliedCoupon) {
      try {
        ...
        console.warn(
          "⚠️ Failed to update coupon usage (non-critical):",
          couponError,
        );
    ...
    if (isConfigError) {
      ...
      userMessage =
        "Ett fel uppstod vid sparande av order. Kontakta support om problemet kvarstår.";
    }

...
function SveaSuccessContent() {
  ...
  const verifyPayment = async (checkoutOrderId: string, orderId: string) => {
    try {
      ...
      if (data.success && data.paymentCompleted) {
        console.log(
          "✅ Payment completed! Setting order details and stopping loader",
        );
        ...
        if (retryCount < maxRetries) {
          setError(
            `Betalningen behandlas fortfarande. Vänligen vänta... (${retryCount + 1}/${maxRetries})`,
          );
          ...
          setError(
            "Betalningen tar längre tid än förväntat. Din order är registrerad och du får ett bekräftelsemail när betalningen är slutförd.",
          );
          setLoading(false);
        }
      } else {
        const errorMsg =
          data.error || data.details || "Kunde inte verifiera betalningen";
      ...
      setError(
        `Ett fel uppstod vid verifiering av betalningen: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
      setLoading(false);
    }
  };
  ...
  const hasCourses = orderDetails?.items?.some(
  ...
  const ebookKey =
    `${purchasedEbook?.productId || ""} ${purchasedEbook?.productName || ""}`.toLowerCase();
  ...
  const ebookDisplay = isPaskbuffe
    ...
    : isSotaGodsaker
      ...
      : isGrillSommarmat
        ? {
            title: "Grill- & Sommarmat – E-bok av Ulrika Davidsson",
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            {item.productType === "course" ? "Kurs" : "Bok"} •{" "}
                            {item.quantity} st
                          </p>
                        </div>
                      </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mt-4">
                <p className="text-sm text-yellow-800">
                  <strong>💡 Tips:</strong> Det kan ta upp till 30 minuter innan
                  mejlet når din inkorg. Kolla även din skräppost-mapp! Mejlet
                  kommer från{" "}
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm mb-2">
                    Ett orderkvitto med dina inloggningsuppgifter skickas till{" "}
                    <strong>{orderDetails?.customerEmail}</strong>. Det kan ta
                    upp till 30 minuter innan mejlet når dig.
                  </p>
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mt-2">
                    <p className="text-sm text-yellow-800">
                      <strong>💡 Tips:</strong> Kolla även din skräppost-mapp!
                      Mejlet kommer från Ulrika Davidsson / Functional Foods.
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600 mt-1">
                        Du får ett separat mejl med nerladdningslänk för din
                        e-bok till {orderDetails?.customerEmail}.
                      </p>
                    </div>
                  </div>
                </div>
